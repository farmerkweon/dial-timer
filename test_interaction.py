...
for lg in T.HELP_LANGS:
    ...
    qrs = [a[2] for a in sup["apps"]]
    check(f"[{lg}] QR 이미지 파일 존재",
          all(os.path.exists(T.resource_path("assets", q)) for q in qrs), qrs)
...
check("앱 버튼이 화면에 그려짐",
      all(a[0] in texts for a in sup_ko["apps"]), )
